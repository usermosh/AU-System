...
<div class="print-only">
  <div style="text-align:center;padding:16px 0 10px;border-bottom:2px solid #4a0e72;margin-bottom:10px">
    ...
    <h3 style="font-size:14px;font-weight:400;margin-bottom:4px">Document Request Transaction Records<?= $filterCourse ? ' — ' . htmlspecialchars($filterCourse) : '' ?><?= $filterYear ? ' — Year ' . $filterYear : '' ?></h3>
    <p style="font-size:10px;color:#555;margin:0">
      Generated: <?= date('F j, Y  h:i A') ?>
      <?= $filterCourse ? ' | Course: ' . htmlspecialchars($filterCourse) : ' | All Courses' ?>
      <?= $filterYear   ? ' | Year Level: Year ' . $filterYear : ' | All Year Levels' ?>
    ...
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td style="text-align:center">Year <?= $r['year_level'] ?></td>
...
<div class="results-bar">
  <div class="results-count">
    ...
    <?php if ($search || $filterType || $filterStatus || $filterMethod || $filterCourse || $filterYear || $filterFrom || $filterTo): ?>
