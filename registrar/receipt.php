...
$stmt = $db->prepare("
    SELECT p.*,
           ...
           s.course, s.year_level, s.section, s.contact_number, s.address,
           u.email,
...
/* ── action banner ── */
.action-banner {
  background:#fffbf0; border:1.5px solid var(--gold);
  border-radius:12px; padding:16px 20px; margin-bottom:22px;
  display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;
}
.action-banner-text { font-size:13px; font-weight:600; color:var(--navy); }
.action-banner-text span { font-size:12px; font-weight:400; color:var(--gray); display:block; margin-top:2px; }
...
<div class="receipt-wrap">
  ...
  <div class="rb" style="position:relative;">
    ...
    <!-- Action banner (pending only, hidden on print) -->
    <?php if ($p['status'] === 'pending'): ?>
    <div class="action-banner no-print">
      <div class="action-banner-text">
        Payment Awaiting Verification
        <span>Review the submitted payment details and approve or reject below.</span>
      </div>
      <div class="action-btns">
        <button class="btn btn-success" onclick="doVerify('verified')">✓ Verify Payment</button>
        <button class="btn btn-danger"  onclick="doVerify('rejected')">✗ Reject</button>
      </div>
    </div>
    <?php endif; ?>
    ...
    <div class="sec">
      ...
      <?php endif; ?>
      <?php if ($p['address']): ?>
      <div class="sec-row"><span class="sec-lbl">Address</span><span class="sec-val"><?= htmlspecialchars($p['address']) ?></span></div>
      <?php endif; ?>
    </div>
