...
@media print {
  ...
  .sidebar, .topbar, .filter-bar, .pagination, .no-print { display:none !important; }
...
<!-- Print Header -->
<div class="print-header print-only" style="text-align:center;padding:20px 0 12px;border-bottom:2px solid #4a0e72;margin-bottom:16px">
  <h2 style="font-family:'Playfair Display',serif;font-size:22px;margin-bottom:4px">Arellano University — Registrar Office</h2>
  <h3 style="font-size:16px;font-weight:400;margin-bottom:4px">Document Request Transaction Records</h3>
...
<div class="results-bar">
  <div class="results-count">
    ...
    <?php if ($search || $filterType || $filterStatus || $filterMethod || $filterFrom || $filterTo): ?>
            ...
            <tr style="<?= $highlight ?>">
              ...
              <td style="font-size:12px;color:var(--gray)">
                ...
                <?php if ($r['processed_at']): ?>
                  <div style="font-size:10px"><?= date('M d H:i', strtotime($r['processed_at'])) ?></div>
                <?php endif; ?>
              </td>
              <td style="font-size:12px;color:var(--gray);white-space:nowrap">
                <?= date('M d, Y', strtotime($r['requested_at'])) ?><br>
                <span style="font-size:11px"><?= date('H:i', strtotime($r['requested_at'])) ?></span>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
...
<div class="print-only" style="margin-top:16px">
  <table class="data-table" style="font-size:10px">
    <thead>
      <tr>
        <th>#</th><th>Student</th><th>Stud. No.</th><th>Course</th>
        <th>Document</th><th>Copies</th><th>Status</th>
        <th>Pay Status</th><th>Amount</th><th>Method</th><th>Ref #</th>
        <th>Purpose</th><th>Processed By</th><th>Requested At</th>
      </tr>
    </thead>
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td><?= date('M d, Y H:i', strtotime($r['requested_at'])) ?></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
